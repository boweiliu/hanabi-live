...
const step1 = (socket, data) => {
  ...
  if (!game.running) {
    logger.warn(
      `messages.action was called for game #${data.gameID}, but it is not started yet.`,
    );
  ...
  if (game.turnPlayerIndex !== data.index) {
    logger.warn(
      `User "${data.username}" tried to perform an action when it was not their turn.`,
    );
  ...
  if (data.type === 0) {
    ...
    if (game.clueNum === 0) {
      logger.warn(
        `User "${data.username}" tried to give a clue while at 0 clues.`,
      );
    ...
    const text = `${
      game.players[game.turnPlayerIndex].username
    } ran out of time!`;
    game.actions.push({
      text,
    });
...
const checkTimer = (data) => {
  ...
  if (data.index === -1) {
    logger.error(
      `Tried to end timed game #${data.gameID}, but the player index could not be found.`,
    );
    return;
  }
  const player = game.players[data.index];
  player.time = 0;
  logger.info(
    `Time ran out for "${player.username}" playing game #${data.gameID}.`,
  );
