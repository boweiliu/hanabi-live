/*
    ...
    message; it can be alerting groups of users about anything.
*/
...
exports.gameMemberChange = (data) => {
  ...
  for (const player of game.players) {
    ...
    // Tell the client to redraw all of the lobby rectangles to account for
    ...
    for (let i = 0; i < game.players.length; i++) {
      ...
      player.socket.emit("message", {
        ...
        resp: {
          ...
          stats: player2.stats, // Defined in the "users" model
        },
      });
    }
  }
...
const playerTable = (socket, data) => {
  ...
  socket.emit("message", {
    ...
    resp: {
      ...
      name: game.name,
      joined,
      numPlayers: game.sharedReplay
        ? Object.keys(game.spectators).length
        : game.players.length,
